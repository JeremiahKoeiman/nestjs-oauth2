  ...
  async authenticate(req: any) {
    ...
    try {
      const oauthUrl = new URL(
        '/oauth/access_token',
        this.config.get('social.facebook.graphUrl'),
      );
      ...
      const {
        data: { access_token },
      } = await this.httpService.get(oauthUrl.toString()).toPromise();

      const fbUserUrl = new URL(
        '/me',
        this.config.get('social.facebook.graphUrl'),
      );
      ...
      const { data } = await this.httpService
        .get<FacebookUserData>(fbUserUrl.toString())
        .toPromise();
